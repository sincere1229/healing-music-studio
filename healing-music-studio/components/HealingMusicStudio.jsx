...
const DURATIONS_P2 = ["10分", "30分", "60分", "120分"];
...
async function renderAudio({ freqs, ambients, instrument, duration }) {
  ...
  const fadeLen = Math.min(8, duration * 0.18);
...
/* =====================================================
   サムネイル生成 (1280×720)
   ===================================================== */
function drawThumbnail({ freqs, use, ambients, durationLabel, mood, uploadedImg }) {
  const W = 1280, H = 720;
  ...
  if (uploadedImg) {
    ...
    c.fillRect(0, 0, W, H);
    // 星雲
    [[W * 0.75, H * 0.3, 340, hue + 30, 0.20], [W * 0.2, H * 0.75, 290, hue - 25, 0.16], [W * 0.5, H * 0.15, 220, 45, 0.07]].forEach(
      ([x, y, r, h2, a]) => {
    ...
    // 三日月
    const mx = W * 0.84, my = H * 0.2, mr = 64;
  ...
  c.textAlign = "center";
  c.fillStyle = "#E8C36A";
  c.font = `500 26px ${serif}`;
  c.fillText("H E A L I N G   M U S I C   S T U D I O", W / 2, 64);

  const freqText = freqs.length ? `${freqs.join("Hz・")}Hz` : "Healing";
  const fgrad = c.createLinearGradient(W / 2 - 300, 0, W / 2 + 300, 0);
  fgrad.addColorStop(0, "#D4AF37");
  fgrad.addColorStop(0.5, "#FBF0CC");
  fgrad.addColorStop(1, "#D4AF37");
  c.fillStyle = fgrad;
  c.font = `700 110px ${serif}`;
  c.shadowColor = "rgba(0,0,0,0.55)";
  c.shadowBlur = 18;
  c.fillText(freqText, W / 2, H * 0.42);

  const useObj = USES.find((u) => u.id === use) || USES[0];
  c.fillStyle = "#FFFFFF";
  c.font = `700 62px ${serif}`;
  c.fillText(`${useObj.label}ヒーリングミュージック`, W / 2, H * 0.58);
  c.shadowBlur = 0;
  ...
  c.fillText([ambLabels && `${ambLabels}の音`, durationLabel].filter(Boolean).join("  ｜  "), W / 2, H * 0.70);

  c.fillStyle = "rgba(232,195,106,0.9)";
  c.font = `500 24px ${serif}`;
  c.fillText("✦ Twinkle Lab ✦", W / 2, H - 50);
...
function buildMusicPrompt({ freqs, use, ambients, instrument, durationLabel, mood }) {
  ...
  return [
    "Deep Ambient Healing Music",
    ...freqs.map((f) => `${f}Hz inspired tone`),
    ...ambients.map((a) => ambMap[a]).filter(Boolean),
    `slow evolving ${instrument === "piano" ? "piano notes" : instrument === "harp" ? "harp arpeggios" : instrument === "strings" ? "string pads" : instrument === "flute" ? "airy flute" : "synth pads"}`,
...
export default function HealingMusicStudio() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setResult({ thumbUrl, wavUrl, videoUrl, videoExt, videoNote, meta, prompt, wavSize: wavBlob.size, videoSize });
      setPhase("done");
    } catch (e) {
      console.error(e);
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: '"Zen Kaku Gothic New", "Hiragino Kaku Gothic ProN", sans-serif' }}>
      ...
      <main className="max-w-3xl mx-auto px-4 py-7 grid gap-4">
        ...
        <Section num="五" title="長さ" sub="MVP版はブラウザ内で生成するため5分まで。10分〜120分はPhase 2(サーバー生成)で対応予定">
          {DURATIONS.map((d) => (
        ...
        {phase === "working" && (
          <div className="rounded-2xl grid gap-2" style={{ background: T.panel, border: `1px solid ${T.borderSoft}`, padding: 18 }}>
            ...
            <p style={{ color: T.sub, fontSize: 11.5 }}>※動画は実時間で録画します({durationLabel}の動画 = 約{durationLabel}の待ち時間)。タブは閉じずにそのままお待ちください。</p>
          </div>
        )}
        ...
        {phase === "done" && result && (
          <div className="grid gap-4">
            <div className="rounded-2xl" style={{ background: T.panel, border: `1px solid ${T.border}`, padding: 20 }}>
              ...
              <p className="mt-4 mb-2 text-sm" style={{ color: T.sub }}>▼ サムネイル(1280×720)</p>
              <img src={result.thumbUrl} alt="サムネイル" className="w-full rounded-xl" style={{ border: `1px solid ${T.borderSoft}` }} />
              ...
              <div className="flex flex-wrap gap-2 mt-5">
                ...
                <a href={result.thumbUrl} download="thumbnail.png" className="rounded-xl text-sm" style={{ padding: "11px 16px", border: `1px solid ${T.border}`, color: T.text }}>
                  ⬇ サムネイル PNG
                </a>
              </div>
